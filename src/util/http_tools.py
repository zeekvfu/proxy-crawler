...
def get_proxy_delay(logger, url, protocol, ip, port, retry=1):
    ...
    for index in range(retry, 0, -1):
        ...
        if result[0] == -1:
            ...
            if isinstance(result[1], (urllib.error.HTTPError, urllib.error.URLError, http.client.InvalidURL, http.client.UnknownProtocol, http.client.LineTooLong, UnicodeError, ssl.CertificateError, TypeError, http.client.HTTPException)):
                break
            else:
                continue
        l.append(result[0])
